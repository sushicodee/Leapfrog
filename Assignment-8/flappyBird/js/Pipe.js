...
class Pipe {
    constructor(dx,dy,dWidth,dHeight,id,state,bird) {    
      ...
      this.domDraw('top-pipe',this.top.sx,this.top.sy,400,-150,this.dWIdth,this.dHeight);
      this.domDraw('bottom-pipe',this.bottom.sx,this.top.sy,400,-150 + this.dHeight+ this.gap,this.dWIdth,this.dHeight);
      this.domDrawScore();
    ...
    draw(){
        ...
        for(let i = 0; i< this.position.length; i++){
          ...
          if(this.state.currentState === this.state.runningState || this.state.currentState === this.state.getReadyState){
    ...
    update(){
      ...
      for(let i =0; i< this.position.length; i++){
        ...
        //check Collision
        // for top
        if(this.bird.dx+this.bird.width > p.dx && this.bird.dx < p.dx + this.dWIdth && this.bird.dy < p.dy + this.dHeight){
           this.HIT_SOUND.play(); 
          this.state.currentState = this.state.gameOverState;
        }
        //for bottom
        if(this.bird.dx+this.bird.width > p.dx && this.bird.dx < p.dx + this.dWIdth && this.bird.dy + this.bird.height > BottomYPos){
            this.HIT_SOUND.play();
          this.state.currentState = this.state.gameOverState;
        }

        //score
        if(this.bird.dx > p.dx + this.dWIdth && this.bird.dx <= p.dx + this.dWIdth + 2){
          this.score++;
